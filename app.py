...
st.set_page_config(
    page_title="Digital Receipt Generator Pro", page_icon="🧾", layout="wide"
)

st.title("🧾 Digital Receipt Generator Pro")
st.caption(
    "Generator kwitansi multi-item dengan pilihan ukuran kertas (A4, A5, Struk"
    " 80mm), custom warna, logo, dan info penjual."
)

# --- SIDEBAR: KONFIGURASI KERTAS & DESAIN ---
...
PAPER_CONFIGS = {
    "A5 Landscape (Default)": {
        ...
        "preview_font_size": "13px",
    },
    "A5 Portrait": {
        "type": "portrait",
        "pagesize": A5,
        "margin": 18,
        "tbl_widths": [25, 170, 30, 75, 75],
        "scale": 0.85,
        "preview_max_width": "500px",
        "preview_font_size": "12px",
    },
    "A4 Landscape": {
        "type": "standard",
...
selected_theme_name = st.sidebar.selectbox(
    "Pilihan Warna Kwitansi", list(COLOR_PRESETS.keys())
)
...
with col_info2:
  ...
  terbilang = st.text_input(
      "Terbilang (Manual/Opsional)", "Tiga Ratus Ribu Rupiah"
  )
...
edited_df = st.data_editor(
    ...
    column_config={
        "Nama Barang": st.column_config.TextColumn(
            "Nama Barang / Deskripsi", required=True
        ),
        "Qty": st.column_config.NumberColumn(
            "Qty", min_value=1, default=1, step=1
        ),
        "Harga Satuan": st.column_config.NumberColumn(
            "Harga Satuan (Rp)", min_value=0, format="Rp %d"
        ),
...
edited_df["Qty"] = (
    pd.to_numeric(edited_df["Qty"], errors="coerce").fillna(0).astype(int)
)
edited_df["Harga Satuan"] = (
    pd.to_numeric(edited_df["Harga Satuan"], errors="coerce")
    .fillna(0)
    .astype(float)
)
...
st.metric(
    "Total Pembayaran Otomatis", f"Rp {total_nominal:,.0f}".replace(",", ".")
)
...
def generate_excel(df_items, total_val):
  ...
  with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
    ...
    fmt_header = workbook.add_format(
        {"bold": True, "bg_color": theme["primary"], "font_color": "white"}
    )
    fmt_bold_currency = workbook.add_format(
        {"bold": True, "num_format": "Rp #,##0"}
    )

    worksheet.write("A1", nama_perusahaan.upper(), fmt_title)
    worksheet.write(
        "A2",
        f"Alamat: {alamat_perusahaan} | Telp: {telepon_perusahaan}",
        fmt_sub,
    )
...
def generate_pdf(df_items, total_val, logo_file, config_key):
  config = PAPER_CONFIGS[config_key]
  buffer = io.BytesIO()
  ...
  if config["type"] == "struk":
    ...
    pagesize = (80 * mm, max(110 * mm, calculated_h))
  else:
  ...
  style_comp_addr = ParagraphStyle(
      "CompAddr",
      parent=styles["Normal"],
      fontName="Helvetica",
      fontSize=6.5 * s,
  ...
  style_terbilang = ParagraphStyle(
      "Terbilang",
      parent=styles["Normal"],
  ...
  style_tbl_cell_bold_r = ParagraphStyle(
      "TblCellBR",
      parent=styles["Normal"],
      fontName="Helvetica-Bold",
      fontSize=8.5 * s,
  ...
  company_cell = [
      ...
      Paragraph(
          f"{alamat_perusahaan} | Telp: {telepon_perusahaan}", style_comp_addr
      ),
  ...
  if logo_file:
    ...
    header_left = Table(
        [[rl_img, company_cell]],
        colWidths=[img_w + 4, w_head_left - (img_w + 4)],
    )
  ...
  t_meta = Table(
      meta_data,
      colWidths=[printable_w * 0.22, printable_w * 0.03, printable_w * 0.75],
  )
  t_meta.setStyle(
      TableStyle([
          ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
  ...
  table_items_data.append([
      ...
      "",
      Paragraph(
          f"<b>Rp {total_val:,.0f}</b>".replace(",", "."), style_tbl_cell_bold_r
      ),
  ])
  ...
  t_footer = Table(
      [[t_footer_left, t_footer_right]], colWidths=[w_foot_left, w_foot_right]
  )
  t_footer.setStyle(
      TableStyle([
          ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
pdf_buf = generate_pdf(
    edited_df, total_nominal, uploaded_logo, selected_paper_name
)
